...
async function signupOrLogin(agent) {
  ...
  const su = await agent
    .post('/signup')
    .send({ firstName: 'ADS', lastName: 'Tester', email: TEST_EMAIL, password: TEST_PASS })
  ...
  // Then login
  const li = await agent.post('/login').send({ email: TEST_EMAIL, password: TEST_PASS });
