...
describe('E2E: full flow of ADS', function () {
  ...
  afterEach(async function () {
    if (this.currentTest?.state === 'failed' && page) {
      ...
      const file = path.join(
        dir,
        `FAILED_${Date.now()}_${this.currentTest.title.replace(/\s+/g, '_')}.png`
      );
  ...
  // --- Helpers -----
  async function doSignupAttempt(email, password, first = 'Christo', last = 'Raju') {
    ...
    if (!/\/login\.html$/.test(page.url())) {
      const msg = await page.locator('#signup-msg').innerText().catch(() => '');
  ...
  async function waitForMeEmail(expectedEmail) {
    ...
    assert.strictEqual(finalMe?.user?.email || null, expectedEmail, 'email should match');
    return finalMe.user;
  }
  ...
  async function browserLogSession() {
    ...
    await page.evaluate(async () => {
      ...
      console.log('[session]', { fullName: d?.user?.fullName, email: d?.user?.email, jti: d?.user?.jti });
    });
  }
  ...
  it('1) Signup → Login → land on index and verify session (then logout)', async () => {
    ...
    // Confirm UI & session
    const logoutVisible = await page.isVisible('form[action="/auth/logout"] button');
  ...
  it('3) Main page: trending → save → My List → Item (optional) → back to index', async () => {
    ...
    const firstDetails = page.locator('#list-collection a[href^="/item?id="]').first();
    if (await firstDetails.count()) {
      ...
      } catch { /* item sections optional; ignore if missing */ }
  ...
  it('5) Chatbot: home → create conversation → chat shell → (optional) bot reply', async () => {
    ...
    await page.waitForSelector('#homeView', { state: 'visible', timeout: 15_000 });

    await page.fill('#homeInput', 'Hi, what’s a good multivitamin?');
    await page.click('#homeSend');

    await page.waitForSelector('#chatShell', { state: 'visible', timeout: 15_000 });
    ...
    // bot reply
    try {
      await page.waitForSelector('#chatArea .message:not(.me) .body', { timeout: 6000 });
    } catch {}
  ...
  it('6) Final logout → login.html and /api/me defaults', async () => {
    ...
    assert.strictEqual(me.user?.email ?? null, null, 'email should be null after logout');
